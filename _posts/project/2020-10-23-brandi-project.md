...
여성 고객을 타겟으로 한 <b>'브랜디'</b>와 남성 고객을 타겟으로 한 <b>'하이버'</b>, 판매자들의 운영상 어려움까지 해결해주는 <b>'헬피'</b> 까지 만들어 운영 중에 있습니다. 
...
#...(생략)
    ...
    session.close()
```
위 코드와 같이 에러가 발생할 경우 에러 메세지를 반환하도록 하고 세션을 롤백 시키도록 해주었습니다. 이때 데이터베이스에 상품 데이터는 저장되지 않지만 서버에 업로드된 이미지에 대한 처리가 없어 그대로 저장된 상태인 것을 알게 되었습니다. 따라서 <b>에러 발생 시 세션 롤백과 함께 S3 서버의 이미지를 삭제하는 로직</b>을 구현해주었습니다!
```py
